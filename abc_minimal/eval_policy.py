...
class PutBottlesEvaluator:
    # Hardened scoring: a bottle counts as PLACED only after being continuously
    # in-bin for PERSIST_STEPS consecutive evaluate() calls (0.5 s at 30 Hz) —
    # single-frame flickers (spawn bounces, gripper swing-throughs) never
    # qualify. The first GRACE_STEPS are ignored entirely (spawn settling).
    # Per-bottle entry/exit events are recorded so any future re-scoring is a
    # replay of stored events, not a re-run.
    PERSIST_STEPS = 15
    GRACE_STEPS = 30
    ...
    def __init__(self, model: mujoco.MjModel, scene: PutBottlesSimConfig):
        ...
        self._step = 0
        self._streak = np.zeros(len(self.bottle_names), dtype=np.int32)
        self._placed_ever = np.zeros(len(self.bottle_names), dtype=bool)
        self._placed_step = {}          # bottle idx -> step of qualifying window START
        self.inbin_events = []          # (step, bottle_name, "enter"|"exit") raw, ungated
        self._prev_inbin = np.zeros(len(self.bottle_names), dtype=bool)
    ...
    def reset(self) -> None:
        ...
        self._step = 0
        self._streak[:] = 0
        self._placed_ever[:] = False
        self._placed_step = {}
        self.inbin_events = []
        self._prev_inbin[:] = False
    ...
    def evaluate(self, qpos: np.ndarray) -> dict[str, Any]:
        ...
        # stash for the caller's state-trace recorder (already fetched — free)
        self.last_qpos = qpos
        ...
        # raw entry/exit events (ungated) for offline re-scoring
        for i in np.flatnonzero(in_bin != self._prev_inbin):
            self.inbin_events.append(
                (self._step, self.bottle_names[i], "enter" if in_bin[i] else "exit")
            )
        self._prev_inbin = in_bin.copy()
        # hardened placement: persistence + spawn grace
        if self._step >= self.GRACE_STEPS:
            self._streak = np.where(in_bin, self._streak + 1, 0)
            newly = (self._streak >= self.PERSIST_STEPS) & ~self._placed_ever
            for i in np.flatnonzero(newly):
                self._placed_ever[i] = True
                self._placed_step[int(i)] = self._step - self.PERSIST_STEPS + 1
        else:
            self._streak[:] = 0
        self._step += 1
        placed = int(self._placed_ever.sum())
        self.max_bottles = max(self.max_bottles, placed)
        success = placed == active
        self.ever_success = self.ever_success or success
        return {
            "reward": float(placed / max(active, 1)),
            "success": bool(num == active),
            ...
            "placed_ever": placed,
            "final_standing": num,
            "placed_steps_per_bottle": {self.bottle_names[k]: int(v) for k, v in self._placed_step.items()},
...
def _save_qpos_trace(
    out_dir: Path, world_index: int, seed: int, trace: list[np.ndarray]
) -> str | None:
    """Persist the per-step qpos trace (fp16 npz). With the world's stored
    randomization (scales -> scene_xml) this makes every rollout offline
    re-renderable and re-scorable without re-running the policy."""
    if not trace:
        return None
    path = out_dir / f"qpos_trace_{world_index:03d}_s{seed}.npz"
    np.savez_compressed(path, qpos=np.stack(trace))
    return str(path)
...
class Pi0Policy:
    ...
    def __init__(self, host: str, port: int, *, prompt: str = PI0_PROMPT):
        ...
        self.prompt = prompt
        _ns = os.environ.get("PI0_NOISE_SEED")
        self._noise_seed = int(_ns) if _ns else None
        self._noise_ah = int(os.environ.get("PI0_NOISE_AH", "16"))
        self._noise_ad = int(os.environ.get("PI0_NOISE_AD", "32"))
        self._noise_scale = float(os.environ.get("PI0_NOISE_SCALE", "1.0"))
        self._noise_mode = os.environ.get("PI0_NOISE_MODE", "full")
        self._noise_log_path = os.environ.get("PI0_NOISE_LOG")
        self._noise_log: list = []
        self._infer_calls = 0
        if self._noise_seed is not None:
            print(f"Pi0Policy: client-side noise ON seed={self._noise_seed} "
                  f"shape=({self._noise_ah},{self._noise_ad}) scale={self._noise_scale}", flush=True)
        if self._noise_log_path is not None:
            import atexit
            atexit.register(self._flush_noise_log)
    ...
    def _flush_noise_log(self) -> None:
        if self._noise_log_path is None or not self._noise_log:
            return
        np.savez_compressed(self._noise_log_path,
                            noise=np.stack(self._noise_log).astype(np.float32),
                            mode=self._noise_mode,
                            seed=self._noise_seed if self._noise_seed is not None else -1)
    ...
    def infer(self, obs: dict[str, Any], noise: np.ndarray | None = None) -> np.ndarray:
        # DSRL prescreen seam: when PI0_NOISE_SEED is set, send an explicit
        # per-chunk initial noise to the server (policy.py forwards it to
        # sample_actions). Unset -> byte-identical to the old behaviour.
        images = obs["images"]
        ...
        if self._noise_seed is not None:
            import zlib
            if noise is not None:
                # inherit the loop's (policy_seed, world, chunk) determinism
                sub = zlib.crc32(np.ascontiguousarray(noise).tobytes())
            else:
                sub = self._infer_calls
            rng = np.random.default_rng([self._noise_seed, sub])
            if self._noise_mode == "repeat32":
                # actor-space noise: one per-action-dim vector tiled across the
                # chunk (the DSRL plan's low-dim parameterization)
                vec = rng.standard_normal(self._noise_ad) * self._noise_scale
                w = np.tile(vec[None, :], (self._noise_ah, 1)).astype(np.float32)
            else:
                w = (rng.standard_normal((self._noise_ah, self._noise_ad))
                     * self._noise_scale).astype(np.float32)
            server_obs["noise"] = w
            if self._noise_log_path is not None:
                # log the compact form: repeat32 stores (32,), full stores flat
                self._noise_log.append(
                    w[0] if self._noise_mode == "repeat32" else w.reshape(-1))
                if len(self._noise_log) % 50 == 0:
                    self._flush_noise_log()
        self._infer_calls += 1
...
def run_eval(config: SimEvalConfig) -> dict[str, Any]:
    ...
    if pi0_mode:
        ...
        # The 30/60 pair below assumes a horizon-30 policy. A horizon-16
        # policy returns only 16 actions per chunk, so 60 chunks would cover
        # 960 steps (32 s) instead of 1800 (60 s) and every score would be
        # measured on a truncated episode. Hold the 1800-action BUDGET fixed
        # and derive the chunk count from the real horizon instead.
        _ecd = int(os.environ.get("PI0_EXECUTE_CHUNK_DIM", "30"))
        _nch = int(os.environ.get("PI0_NUM_CHUNKS", str(-(-1800 // _ecd))))
        logging.info(
            "pi0 rollout budget: execute_chunk_dim=%d x num_chunks=%d = %d actions "
            "(target 1800 = 60 s @ 30 Hz)", _ecd, _nch, _ecd * _nch,
        )
        config = replace(
            config,
            execute_chunk_dim=_ecd,
    ...
    if pi0_mode:
        ...
        policy = Pi0Policy(
            config.pi0_host,
            config.pi0_port,
            prompt=config.pi0_prompt if config.pi0_prompt is not None else PI0_PROMPT,
        )
    else:
    ...
    try:
        for world_index in range(config.num_worlds):
            ...
            steps = 0
            chunk_metrics = []
            # per-step placement timestamps (step index when the k-th bottle first
            # registers in-bin; eval_fn already runs every step, so this is free and
            # gives 1/30 s resolution vs the 1-chunk resolution of chunk_metrics)
            placement_steps: list[int] = []
            # full qpos trace (fp16) -> offline re-render / re-score of the EXACT rollout
            ...
            try:
                ...
                for chunk in range(config.num_chunks):
                    ...
                    for action_index, action in enumerate(actions[: config.execute_chunk_dim]):
                        ...
                        final_eval = eval_fn()
                        steps += 1
                        qpos_trace.append(env.evaluator.last_qpos.astype(np.float16))
                        while len(placement_steps) < int(final_eval["max_bottles_in_bin_so_far"]):
                            placement_steps.append(steps)
                        ...
                        if config.early_stop and final_eval["ever_success"]:
                            break
            ...
            world = {
                ...
                "placement_times_s": [round(s / 30.0, 4) for s in placement_steps],
                "inbin_events": [
                    [int(st), nm, ev] for st, nm, ev in env.evaluator.inbin_events
                ],
                "scorer": {
                    "persist_steps": PutBottlesEvaluator.PERSIST_STEPS,
                    "grace_steps": PutBottlesEvaluator.GRACE_STEPS,
                },
                "qpos_trace_path": _save_qpos_trace(out_dir, world_index, seed, qpos_trace),
